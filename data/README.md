...
| `soil_cores_raw.csv` | table | 2 | Raw depth-interval measurements feeding the depth-harmonization step. The synthetic version uses `plot_id, depth_from, depth_to, soc, bulk_density, coarse_frag` — **this is the biggest unknown**: `soilassessment::depthharm()`'s real expected column names/shape haven't been verified against the installed package version (flagged in `R/step02_harmonize_depths.R` too). Send a few real rows and we'll match the function to them, or hand-calculate one core per the workshop's Step 2 first. |
